...
Scenario: View salaries using the widget
	When user selected 'Дніпро' in City drop-down menu 
...
Scenario: View salary dynamics
	...
	And user selected city 'Дніпро' in Dynamics of programmers salaries drop-down menu
	Then user sees the dynamics of salaries in the city 'Дніпро'


Scenario: View salaries by city
	When user clicks on the By cities button
	And user selected period 'грудень 2020' in Salaries by cities drop-down menu
	Then user sees the salary by city for the period 'грудень 2020'


Scenario: View salary survey demographics
	When user clicks on the Demographics button 
	And user selected period 'грудень 2019' in Demography of the salary survey drop-down menu
	Then user then sees the payroll survey demographics for 'грудень 2019'
